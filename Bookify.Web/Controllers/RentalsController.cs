    ...
    public class RentalsController : Controller
    {
        ...
        public IActionResult Return(int id)
        {
            ...
            var viewModel = new RentalReturnFormViewModel
            {
                ...
                AllowExtend = !subscriber!.IsBlackListed
                    && subscriber!.Subscribtions.Last().EndDate >= rental.StartDate.AddDays((int)RentalsConfigurations.MaxRentalDuration)
        ...
        public IActionResult Return(RentalReturnFormViewModel model)
        {
            ...
            if (model.SelectedCopies.Any(c => c.IsReturned.HasValue && !c.IsReturned.Value))
            {
                ...
                else if (subscriber!.Subscribtions.Last().EndDate < rental.StartDate.AddDays((int)RentalsConfigurations.MaxRentalDuration))
        ...
        private (string errorMessage, int? maxAllowedCopies) ValidateSubscriber(Subscriber subscriber, int? rentalId = null)
        {
            ...
            if (subscriber.Subscribtions.Last().EndDate < DateTime.Today.AddDays((int)RentalsConfigurations.RentalDuration))
